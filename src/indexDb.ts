...
class Transaction {
  ...
  protected show: HTMLDivElement;
  constructor(file:HTMLInputElement ,show:HTMLDivElement){
    this.file=file
    this.show=show
  ...
  addData(e: Event) {
    ...
    for (let i = 0; i < this.file?.files!.length; i++) {
      ...
      reader.addEventListener("load", () => {
        ...
        const transaction:IDBTransaction = db
          .transaction(["image"], "readwrite");
          const request:IDBRequest=transaction.objectStore("image")
          .add(base64);
